...
1. **全文稿（修正版）**：纠正姓名、术语、误听与断句，保留全部发言和原时间戳，记录重要修正及疑点。
   ...
   **Corrected transcript**: Fix names, terminology, misheard words, punctuation, and sentence boundaries while preserving every speaking turn and original timestamp. Record material corrections and unresolved issues.

2. **Shownotes**：核心问题型标题、节目介绍、关键词、嘉宾主持、主题时间轴和已有文字稿入口。

   **Show notes**: An episode title built around a central question, an introduction, keywords, guest and host information, a topic timeline, and links to available transcripts or articles.

3. **精华稿**：独立文章标题、导语、目录、主题分章与精编问答，保留故事和论证过程。

   **Edited highlights article**: A standalone title, introduction, table of contents, thematic sections, and edited Q&A that preserves the stories and reasoning behind the ideas.
...
## 输入、语言与文件名 / Inputs, languages, and filenames
...
- `SKILL.md`：技能入口，包含三份产出的完整流程。 / The skill entry point and complete workflow for all three outputs.
- `agents/openai.yaml`：可选的 Codex 界面元信息。 / Optional Codex interface metadata.
- `references/transcript-correction.md`：纠错、核验、时间戳和完整性要求。 / Rules for correction, verification, timestamps, and completeness.
- `references/editorial-style.md`：从 E81 提炼的编辑结构与文风。 / Editorial structure and style generalized from E81.
- `assets/`：三份可复用 Markdown 骨架。 / Reusable Markdown templates for the three documents.
